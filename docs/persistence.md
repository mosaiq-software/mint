...
It is best to try to avoid this. Note that versioning is not necessary for changes to these types that are purely additive, such as adding _optional_ properties to `Document` or a new kind of layer to the compound `Layer` type. In other words, if the old `Document` or `Layer` type could be safely cast to the new, it is not necessary to re-version.
